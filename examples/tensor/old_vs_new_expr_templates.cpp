...
decltype(auto) make_squared_difference(T1 &&t1, T2 &&t2) {
  static_assert(boost::numeric::ublas::detail::is_tensor_v<std::remove_reference_t<T1>> &&
      boost::numeric::ublas::detail::is_tensor_v<std::remove_reference_t<T2>>, "Arguments must be tensor type");
...
int main() {
  ...
  using shape = boost::numeric::ublas::dynamic_extents<>;
...

  {
    ...
    auto expr = boost::numeric::ublas::apply( a+b ,[] (auto const& val) { return sqrt(val); }); // ==> sqrt(a+b); All Lazy
    tensor_float x = expr;
