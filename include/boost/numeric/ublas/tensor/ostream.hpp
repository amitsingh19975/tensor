...
template <class value_type> void print(std::ostream &out, value_type const &p) {
  out << p << " ";
}
...
void print(std::ostream &out, size_type r, const value_type *p,
           const size_type *w, const size_type *n) {

  if (r < 2) {
    out << "[ ... " << std::endl;

    for (auto row = 0u; row < n[0];
         p += w[0], ++row) { // iterate over one column
      auto p1 = p;
      for (auto col = 0u; col < n[1];
           p1 += w[1], ++col) { // iterate over first row
        print(out, *p1);
      }
      if (row < n[0] - 1)
        out << "; " << std::endl;
...
std::ostream &operator<<(std::ostream &out,
                         boost::numeric::ublas::tensor<V, E, F, A> const &t) {
  ...
  if (is_scalar(t.extents())) {
    ...
    if constexpr (boost::numeric::ublas::detail::is_static_extents<E>::value) {
      ...
      boost::numeric::ublas::detail::print(out, t.rank() - 1, t.data(),
                                           s.data(), e.data());
    } else {
      boost::numeric::ublas::detail::print(
          out, t.rank() - 1, t.data(), t.strides().data(), t.extents().data());
    }
  }
  return out;
}
