...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_expression_for_each_tensor, value,
  ...
  for (auto const &e : extents) {
    ...
    BOOST_TEST_PASSPOINT();
    auto transformed_expr1 = ublas::for_each2(std::move(terminal_tensor), [](auto const& ep){return 5.0f;});
//    BOOST_TEST_PASSPOINT();
...
//    auto transformed_expr4 = ublas::for_each2(terminal_tensor, [](value_type const& ep){return sqrt(ep);});
    ...
    BOOST_TEST_PASSPOINT();
    auto transformed_expr5 = ublas::for_each2(std::move(t_copy2), [](auto const& ep){return 5.0f;});
//    BOOST_TEST_PASSPOINT();
