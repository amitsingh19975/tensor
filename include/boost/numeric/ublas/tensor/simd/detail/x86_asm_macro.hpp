#if !defined(x86_ASM_MACRO_HPP)
#define x86_ASM_MACRO_HPP

#define ASM_BEGIN() asm volatile (
#define ASM_END(...) __VA_ARGS__ );

#define asm_begin() ASM_BEGIN()
#define asm_end(...) ASM_END(__VA_ARGS__)
...
#define LABEL(label) LABEL_(label) ":\n\t"
#define REGISTER_(r) %%r
#define IMM(x) $##x
#define VAR(x) %[x]
#define MASK(x) %{x%}
#define JMP_(ins, target) STRINGIFY_(ins) " " LABEL_(target) "\n\t"
