...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_expression_apply_tensor, value,
                                 test_types, fixture) {
  ...
  for (auto const &e : extents) {
    ...
    if (!e.empty()) {
      ...
      auto terminal_tensor = boost::yap::make_terminal<ublas::detail::tensor_expression>(t);
          ...
          auto transformed_expr1 = ublas::apply(terminal_tensor, [](auto const& ep){return 5.0f;});
          auto transformed_expr2 = ublas::apply(terminal_tensor, [](auto const& ep){return 5.0f+ep;});
          auto transformed_expr3 = ublas::apply(terminal_tensor, [](auto const& ep){return ep*ep;});
          auto transformed_expr4 = ublas::apply(terminal_tensor, [](value_type const& ep){return sqrt(ep);});

          auto transformed_expr5 = ublas::apply(t_copy2, [](auto const& ep){return 5.0f;});
          auto transformed_expr6 = ublas::apply(t_copy2, [](auto const& ep){return 5.0f+ep;});
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_expression_apply_expression,
  ...
  for (auto const &e : extents) {
    ...
    if(!e.empty()) {
      ...
      auto m_expr2 = ublas::apply(expr, [](auto const &e) { return e * e; });

      auto m_expr3 = ublas::apply(expr, [](auto const &e) {
        return value_type{2} * e - value_type{8};
      });

      auto m_expr4 = ublas::apply(expr, [](auto const &e) {
        return e == value_type{0} ? value_type{1} : value_type{0};
      }); // compliment function

      auto m_expr5 = ublas::apply(expr, [](auto const &e) { return value_type{1}; });
      ...
      std::for_each(k.begin(), k.end(), [](auto &x) {
        if (x == value_type{0})
          x = value_type{1};
        else
          x = value_type{0};
