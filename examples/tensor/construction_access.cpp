...
int main()
{
...

	{
		...
		// to the first-order storage format
		using ftype = float;
		auto A = tensor<ftype,static_extents<3,4,2>>{};
		...
		// and initializes it with the default value.
		using ctype = std::complex<cpp_bin_float_double_extended>;
		using extents_type = static_extents<5,4,3,2>;
		auto B = tensor<ctype,extents_type,last_order>(extents_type{},ctype{});
		...
		auto ex = B.extents();
		auto C = tensor<ctype,dynamic_extents<>,last_order>(ex.to_dynamic_extents());
		// computes the complex conjugate of elements of B
		// using multi-index notation.
		for(auto i = 0u; i < B.size(0); ++i)
			for(auto j = 0u; j < B.size(1); ++j)
		...
		// computes the complex conjugate of elements of B
		// using iterators.
		auto D = tensor<ctype,dynamic_extents<>,last_order>(ex.to_dynamic_extents());
		std::transform(B.begin(), B.end(), D.begin(), [](auto const& b){ return std::conj(b); });
		std::cout << "% --------------------------- " << std::endl;
		std::cout << "% --------------------------- " << std::endl << std::endl;
		std::cout << "D=" << D << ";" << std::endl << std::endl;
