...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_expression_for_each_tensor, value,
  ...
  for (auto const &e : extents) {
    ...
    BOOST_TEST_MESSAGE("Extent on for_each tensor : "+ e.to_string());
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_expression_for_each_expression,
  ...
  for (auto const &e : extents) {
    ...
    BOOST_TEST_MESSAGE("Extent on for_each expr : "+ e.to_string());
    ...
    if(!e.empty()) {
      ...
      auto m_expr3 = ublas::for_each(expr, [](auto const &e) {
        return value_type{2} * e - value_type{8};
      });

      auto m_expr4 = ublas::for_each(expr, [](auto const &e) {
