...
template <class int_type> class basic_extents {
  static_assert(std::numeric_limits<
                    typename std::vector<int_type>::value_type>::is_integer,
                "Static error in basic_layout: type must be of type integer.");
  static_assert(
      !std::numeric_limits<
          typename std::vector<int_type>::value_type>::is_signed,
      "Static error in basic_layout: type must be of type unsigned integer.");
...
public:
   ...
   */
  constexpr explicit basic_extents() : _base{} {}
   ...
   */
  explicit basic_extents(base_type const &b) : _base(b) {
    if (!this->valid()) {
      throw std::length_error(
          "Error in basic_extents::basic_extents() : shape tuple is not a "
          "valid permutation: has zero elements.");
    }
  }
   ...
   */
  explicit basic_extents(base_type &&b) : _base(std::move(b)) {
    if (!this->valid()) {
      throw std::length_error(
          "Error in basic_extents::basic_extents() : shape tuple is not a "
          "valid permutation: has zero elements.");
  ...
  basic_extents(basic_extents const &l) : _base(l._base) {}

  /** @brief Move constructs basic_extents */
  basic_extents(basic_extents &&l) noexcept : _base(std::move(l._base)) {}
   ...
   * @returns true if (1)
   */
  bool is_free_scalar() const { return _base.size() == 1 && _base[0] == 1; }
  ...
  const_pointer data() const { return this->_base.data(); }

  const_reference operator[](size_type p) const { return this->_base[p]; }

  const_reference at(size_type p) const { return this->_base.at(p); }

  reference operator[](size_type p) { return this->_base[p]; }

  reference at(size_type p) { return this->_base.at(p); }
  ...
  size_type product() const {
    ...
    return std::accumulate(_base.begin(), _base.end(), 1ul,
                           std::multiplies<>());
  }
  ...
  basic_extents squeeze() const {
    ...
    auto insert_iter =
        std::back_insert_iterator<typename basic_extents::base_type>(
            new_extent._base);
    std::remove_copy(this->_base.begin(), this->_base.end(), insert_iter,
                     value_type{1});
    return new_extent;
  }

  void clear() { this->_base.clear(); }

  bool operator==(basic_extents const &b) const { return _base == b._base; }
  ...
  /** @brief Returns a string representation of the shape/extents
   *   shape{2,3,4} -> "{2, 3, 4}"
   *   shape{2,dynamic,5} -> "{2, -1, 5}"
   */
  ...
  std::string to_string() const {
    ...
    res += std::to_string(_base[_base.size() - 1]) + "}";
    return res;
  }
