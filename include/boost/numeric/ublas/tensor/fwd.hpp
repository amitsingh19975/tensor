...
template <class E, typename> auto to_string(E const &e);
...
template <typename T, typename... S> struct subtensor;
...
template <bool, typename E, typename... S> struct default_span_array_impl;

template <typename E, typename... S> struct default_span_arary;

template <typename T> struct sub_extents;

template <typename T, typename L> struct sub_strides;

template <typename T, typename E, typename L> struct sub_span_stride;
...
{
template <typename T, typename E, typename A, typename = void> struct default_storage;
}
...
template <size_t I> struct index_type;
}
...
template <typename... Ts> struct slice_common_type;

template <typename T, ptrdiff_t f_, ptrdiff_t l_, ptrdiff_t s_, ptrdiff_t sz> struct normalized_slice;
