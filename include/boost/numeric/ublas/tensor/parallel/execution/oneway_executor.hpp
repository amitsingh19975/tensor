#ifndef _BOOST_UBLAS_TENSOR_PARALLEL_EXECUTION_ONEWAY_EXECUTOR_HPP
#define _BOOST_UBLAS_TENSOR_PARALLEL_EXECUTION_ONEWAY_EXECUTOR_HPP
...
namespace boost::numeric::ublas::parallel::execution{
    ...
    namespace detail::oneway_executor
    {
        ...
        struct single_use_func : func_base< R, Args... >{
            
            single_use_func( Function fn )
                : m_func(std::move(fn))
            {}
        ...
        using oneway_func_base = func_base<void>;
        template<typename F> using oneway_func = single_use_func<F, void>;
        ...
        struct impl : impl_base {
            ...
            virtual void execute(std::unique_ptr<oneway_func_base> f){
                m_executor.execute([fn = std::move(f)]() mutable {
                    fn.release()->call();
                });
            }
        ...
        private:
            ...
            impl_base* require_helper(property_list<Properties...>, std::type_info const& t, void const* p ){
                if constexpr( sizeof...(Properties) == 0 ){
                    TENSOR_ASSERT(0,"boost::numeric::ublas::parallel::execution::detail::oneway_executor::impl_base::require_helper(): property_list is empty");
                    return nullptr;
                }else{
            ...
            impl_base* require_conecpt_helper(property_list<Properties...>, std::type_info const& t, void const* p ){
                if constexpr( sizeof...(Properties) == 0 ){
                    TENSOR_ASSERT(0,"boost::numeric::ublas::parallel::execution::detail::oneway_executor::impl_base::require_conecpt_helper(): property_list is empty");
                    return nullptr;
                }else{
                    using List = property_list<Properties...>;
                    using Head = decltype( peek_front(List{}) );
    ...
    struct oneway_t::polymorphic_executor_type{
        ...
        polymorphic_executor_type( Executor e, 
            std::enable_if_t< detail::oneway_executor::is_valid_target_v<
        ...
        )
        {
            ...
            m_impl = new detail::oneway_executor::impl<decltype(new_e),SupportableProperties...>(std::move(new_e));
        }
        ...
        polymorphic_executor_type( polymorphic_executor_type<OtherSupportableProperties...> const& e, 
            std::enable_if_t< detail::oneway_executor::has_exact_property_list_v<
                detail::oneway_executor::property_list<SupportableProperties...>,
        ...
        template<typename Property, 
            typename = std::enable_if_t<
                detail::oneway_executor::find_convertible_property_t<Property,SupportableProperties...>::is_requirable_concept
        ...
        typename Property::template polymorphic_executor_type<SupportableProperties...> require_concept(Property const& p) const{
            ...
            detail::oneway_executor::find_convertible_property_t<Property,SupportableProperties...> p1(p);
        ...
        template<typename Property, 
            typename = std::enable_if_t<
                detail::oneway_executor::find_convertible_property_t<Property,SupportableProperties...>::is_requirable
                && !is_interface_property_v<Property>
            > >
        ...
        template<typename Property, 
            typename = std::enable_if_t<
                detail::oneway_executor::find_convertible_property_t<Property,SupportableProperties...>::is_preferable >
            >
        ...
        auto query( Property const& p )
            -> typename detail::oneway_executor::find_convertible_property_t<Property, SupportableProperties...>::polymorphic_query_result_type
        {
            detail::oneway_executor::find_convertible_property_t<Property,SupportableProperties...> p1(p);
        ...
        void execute( Function f ) const{
            std::unique_ptr<detail::oneway_executor::oneway_func_base> fp(new detail::oneway_executor::oneway_func<Function>(std::move(f)));
            m_impl ? m_impl->execute(std::move(fp)) : throw bad_executor();
        }
    ...
    private:
        template<typename...> friend struct oneway_t::polymorphic_executor_type;
        polymorphic_executor_type(detail::oneway_executor::impl_base* i) noexcept : m_impl(i){}
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE detail::oneway_executor::impl_base const* get_impl() const noexcept { return m_impl; }
        
    private:
        detail::oneway_executor::impl_base* m_impl{nullptr};
    };
