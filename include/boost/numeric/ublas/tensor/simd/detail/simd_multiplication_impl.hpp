#ifndef _BOOST_UBLAS_TENSOR_DETAIL_SIMD_MUTLTIPLICATION_IMPL_HPP
#define _BOOST_UBLAS_TENSOR_DETAIL_SIMD_MUTLTIPLICATION_IMPL_HPP
...
namespace boost::numeric::ublas::simd::detail{
    ...
    BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto cast_to_simd_int64(U const& old) noexcept{
        auto* temp = reinterpret_cast<int64_t const*>(&old);
        if constexpr( N == 128 ){
            return assignment<128,double>{}(temp[0], temp[1]);
        }else if constexpr( N == 256 ){
            return assignment<256,double>{}(temp[0], temp[1], temp[2], temp[3]);
        }else if constexpr( N == 512 ){
            return assignment<512,double>{}(temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6], temp[7]);
        }
    }

    template<size_t N, typename U>
    BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto cast_to_simd_double(U const& old) noexcept{
        auto* temp = reinterpret_cast<double const*>(&old);
        if constexpr( N == 128 ){
            return assignment<128,int64_t>{}(temp[0], temp[1]);
        }else if constexpr( N == 256 ){
            return assignment<256,int64_t>{}(temp[0], temp[1], temp[2], temp[3]);
        }else if constexpr( N == 512 ){
            return assignment<512,int64_t>{}(temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6], temp[7]);
        }
    }
    ...
    struct multiplies{
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto operator()(simd_type_t<128,U> const& a, simd_type_t<128,U> const& b) const noexcept{
            if constexpr(std::is_same_v<T,int64_t> || std::is_same_v<T,long long>){
                ...
                auto rl = _mm_cvtps_epi32( _mm_mul_ps(al,bl) );
                auto rh = _mm_cvtps_epi32( _mm_mul_ps(ah,bh) );
                ...
                auto r0 = _mm_cvtps_epi32( _mm_mul_ps(a0,b0) );
                auto r1 = _mm_cvtps_epi32( _mm_mul_ps(a1,b1) );
                auto r2 = _mm_cvtps_epi32( _mm_mul_ps(a2,b2) );
                auto r3 = _mm_cvtps_epi32( _mm_mul_ps(a3,b3) );
                ...
                float res[bsz];
                std::transform(na, na + bsz, nb, res, std::multiplies{} );
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto operator()(simd_type_t<256,U> const& a, simd_type_t<256,U> const& b) const noexcept{
            if constexpr(std::is_same_v<T,int64_t> || std::is_same_v<T,long long>){
                ...
                auto r0 = _mm256_cvtps_epi32( _mm256_mul_ps(a0,b0) );
                auto r1 = _mm256_cvtps_epi32( _mm256_mul_ps(a1,b1) );
                auto r2 = _mm256_cvtps_epi32( _mm256_mul_ps(a2,b2) );
                auto r3 = _mm256_cvtps_epi32( _mm256_mul_ps(a3,b3) );
                ...
                auto r2 = _mm256_cvtps_epi32( _mm256_mul_ps(a2,b2) );
                auto r3 = _mm256_cvtps_epi32( _mm256_mul_ps(a3,b3) );
                auto r4 = _mm256_cvtps_epi32( _mm256_mul_ps(a4,b4) );
                auto r5 = _mm256_cvtps_epi32( _mm256_mul_ps(a5,b5) );
                auto r6 = _mm256_cvtps_epi32( _mm256_mul_ps(a6,b6) );
                auto r7 = _mm256_cvtps_epi32( _mm256_mul_ps(a7,b7) );
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto  operator()(simd_type_t<128,float> const& a, simd_type_t<128,float> const& b) const noexcept{
            static_assert(std::is_same_v<T,float>, "boost::numeric::ublas::simd::detail::multiplies::operator()(simd_type_t<128,float>,simd_type_t<128,float>) : invalid type, expected float");
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto  operator()(simd_type_t<256,float> const& a, simd_type_t<256,float> const& b) const noexcept{
            static_assert(std::is_same_v<T,float>, "boost::numeric::ublas::simd::detail::multiplies::operator()(simd_type_t<256,float>,simd_type_t<256,float>) : invalid type, expected float");
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto  operator()(simd_type_t<512,float> const& a, simd_type_t<512,float> const& b) const noexcept{
            static_assert(std::is_same_v<T,float>, "boost::numeric::ublas::simd::detail::multiplies::operator()(simd_type_t<512,float>,simd_type_t<512,float>) : invalid type, expected float");
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto  operator()(simd_type_t<128,double> const& a, simd_type_t<128,double> const& b) const noexcept{
            static_assert(std::is_same_v<T,double>, "boost::numeric::ublas::simd::detail::multiplies::operator()(simd_type_t<128,double>,simd_type_t<128,double>) : invalid type, expected double");
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto  operator()(simd_type_t<256,double> const& a, simd_type_t<256,double> const& b) const noexcept{
            static_assert(std::is_same_v<T,double>, "boost::numeric::ublas::simd::detail::multiplies::operator()(simd_type_t<256,double>,simd_type_t<256,double>) : invalid type, expected double");
        ...
        BOOST_UBLAS_TENSOR_ALWAYS_INLINE auto  operator()(simd_type_t<512,double> const& a, simd_type_t<512,double> const& b) const noexcept{
            static_assert(std::is_same_v<T,double>, "boost::numeric::ublas::simd::detail::multiplies::operator()(simd_type_t<512,double>,simd_type_t<512,double>) : invalid type, expected double");
