...
#ifndef BOOST_UBLAS_TENSOR_PARALLEL_MULTIPLICATION
...
namespace boost::numeric::ublas::parallel::detail::recursive {
...
// std::array< float, simd::detail::block_size_v<256,float> * simd::detail::block_size_v<256,float> > mtm_res;
template <class PointerOut, class SizeType, class PointerIn>
BOOST_UBLAS_TENSOR_ALWAYS_INLINE void mtm_helper(PointerOut c,
    PointerIn a, PointerIn b, SizeType wr, SizeType wc)
{
	static constexpr auto bsz = simd::detail::block_size_v<256,float>;
	std::array< simd::basic_simd_type<256,float>, bsz > asa;
	std::array< simd::basic_simd_type<256,float>, bsz > asb;

	auto ci = c;

	for( SizeType i = 0; i < bsz; a += wr, ++i ){
		asa[i].load( a, wc );
	}
	for(SizeType j = 0; j < bsz; b += wc, ++j){
		asb[j].load( b, wr );
	}
	...
	for( SizeType i = 0; i < bsz; ai += 1, ci += wr, ++i ){
		auto cj = ci;
		auto bj = bi;
		for(SizeType j = 0; j < bsz; bj += 1, cj += wc, ++j){
			*cj = simd::dot_prod(*ai,*bj);
		}
	}

}

template <class PointerOut, class SizeType, class SimdStorage1, class SimdStorage2, class Executor>
BOOST_UBLAS_TENSOR_ALWAYS_INLINE void mtm_helper(PointerOut c, SizeType const*const nc, SizeType const*const wc,
    SimdStorage1 const& a, SimdStorage2 const& b,
...
void mult_8nx8n(float* c, SizeType const* nc, SizeType const* wc,
    float const* const& a, SizeType const* na, SizeType const* wa,
	float const* const& b, SizeType const* nb, SizeType const* wb,
...
template<typename T>
using remove_cp_t = std::remove_cv_t< std::remove_pointer_t<T> >;
...
*/
template <size_t N = 256, class PointerOut, class PointerIn1, class PointerIn2, class SizeType>
BOOST_UBLAS_TENSOR_ALWAYS_INLINE void mtm(PointerOut c, SizeType const*const nc, SizeType const*const wc,
         PointerIn1 a, SizeType const*const na, SizeType const*const wa,
         PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
...
*/
template <size_t N = 256, class PointerOut, class PointerIn1, class PointerIn2, class SizeType>
BOOST_UBLAS_TENSOR_ALWAYS_INLINE void mtm_simd_only(PointerOut c, SizeType const*const nc, SizeType const*const wc,
         PointerIn1 a, SizeType const*const na, SizeType const*const wa,
         PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
    ...
    simd::mult_8nx8n(c,nc,wc,a,na,wa,b,nb,wb);
