...
int main()
{
...

	{
		// Tensor-Vector-Multiplications - Including Transposition
		{
			...
			auto A = tensor<value_t,decltype(n),format_t>(n,1);
			auto B1 = matrix_t(n[1],n[2],2);
			auto v1 = tensor<value_t,static_extents<n[0],1>,format_t>(static_extents<n[0],1>{},2);
			auto v2 = tensor<value_t,static_extents<n[1],1>,format_t>(static_extents<n[1],1>{},2);
	...
	//		std::cout << "C2=" << C2 << ";" << std::endl << std::endl;
		...
		// Tensor-Matrix-Multiplications - Including Transposition
		{
			...
			auto B  = tensor<value_t,static_extents<n[1],n[2],m>,format_t>(static_extents<n[1],n[2],m>{},2);
			auto B1 = tensor<value_t,static_extents<m,n[0]>,format_t>(static_extents<m,n[0]>{},1);
			auto B2 = tensor<value_t,static_extents<m,n[1]>,format_t>(static_extents<m,n[1]>{},1);
	...
	//		std::cout << "C3=" << C3 << ";" << std::endl << std::endl;
		...
		// Tensor-Tensor-Multiplications Including Transposition
		{
			auto na = static_extents<3,4,5>{};
			auto nb = static_extents<4,6,3,2>{};
			auto A = tensor<value_t,decltype(na),format_t>(na,2);
			auto B = tensor<value_t,decltype(nb),format_t>(nb,3);
			auto T1 = tensor<value_t,static_extents<na[2],na[2]>,format_t>(static_extents<na[2],na[2]>{},2);
			auto T2 = tensor<value_t,static_extents<na[2],nb[1],nb[3]>,format_t>(static_extents<na[2],nb[1],nb[3]>{},2);
