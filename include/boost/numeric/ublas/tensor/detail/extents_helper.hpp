...
struct basic_extents_impl<R, basic_shape<>>
{
  ...
  [[nodiscard]] BOOST_UBLAS_TENSOR_INLINE
  constexpr auto &step(int) noexcept { return this->S; }
  
  [[nodiscard]] BOOST_UBLAS_TENSOR_INLINE
  constexpr auto &step(int) const noexcept { return this->S; }
  /**
   * @returns extent at a given index
   **/
  [[nodiscard]] BOOST_UBLAS_TENSOR_INLINE
  ...
  {
    TENSOR_ASSERT(0,"boost::numeric::ublas::detail::basic_extents_impl: invalid iterator type");
  }
    ...
    : basic_extents_impl<R + 1, basic_shape<E...>>
{
  ...
  explicit constexpr basic_extents_impl(ptrdiff_t extent,
                                        IndexType... DynamicExtents) noexcept(TENSOR_ASSERT_NOEXCEPT)
      : next(DynamicExtents...)
      , N(extent)
  {
    ...
    TENSOR_ASSERT( BOOST_UBLAS_TENSOR_LIKLY( extent > 0 ), "boost::numeric::ublas::detail::basic_extents_impl: extent should be greater than 0");
  }
  ...
  constexpr basic_extents_impl(Iterator begin, Iterator end,
      ...
      , N(*begin) 
  {
    TENSOR_ASSERT( BOOST_UBLAS_TENSOR_LIKLY( *begin > 0 ), "boost::numeric::ublas::detail::basic_extents_impl: extent should be greater than 0");
  }

  template <typename Iterator>
  basic_extents_impl(Iterator, Iterator, invalid_iterator_tag) noexcept(TENSOR_ASSERT_NOEXCEPT)
  {
    TENSOR_ASSERT(0,"boost::numeric::ublas::detail::basic_extents_impl: invalid iterator type");
  }
