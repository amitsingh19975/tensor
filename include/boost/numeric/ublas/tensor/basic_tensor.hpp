...
#ifndef BOOST_UBLAS_BASIC_TENSOR_IMPL_HPP
#define BOOST_UBLAS_BASIC_TENSOR_IMPL_HPP
...
public:
    ...
    static_assert( std::is_same<layout_type,first_order>::value || 
                   ...
                   std::is_same<layout_type,custom_order >::value, 
    ...
    constexpr basic_tensor ()
        : basic_tensor(std::move(extents_type{}),container_tag{})
    {}

    constexpr basic_tensor( extents_type e, tag::dynamic_storage )
    ...
    constexpr basic_tensor( extents_type e, tag::static_storage<> )
        : tensor_expression_type<self_type>()
        , extents_( std::move( e ) )
        , strides_(extents_)
        , size_( product(extents_) )
    {
        if( data_.size() < product(extents_) ){
            throw std::runtime_error("boost::numeric::ublas::basic_tensor(extents_type,tag::static_storage<>): "
                "extents exceeds the buffer size of static storage"
            );
        }
    }
    ...
    const_reference at (size_type i, size_types ... is) const {
        ...
        if constexpr (sizeof...(is) == 0)
            ...
            temp = detail::access<0ul>(size_type(0),this->strides_,i,std::forward<size_types>(is)...);
        }

        if constexpr ( std::is_same_v< custom_order,layout_type > ){
            if( size_ <= temp ){
                throw std::out_of_range("boost::numeric::ublas::tensor::at(size_type,...) : "
                    "out of bound access"
                );
            }
    ...
    const_iterator end () const {
        auto temp = data_.begin();
        std::advance( temp, size_ );
        return temp;
    }
    ...
    const_reverse_iterator rbegin () const {
        auto temp = data_.rbegin();
        std::advance( temp, data_.size() - size_ );
        return temp;
    }
