...
/** @brief Returns true if size > 1 and all elements > 0 or size == 1 && e[0] ==
 * 1 */
template <class E,
          typename std::enable_if<detail::is_extents<E>::value, int>::type = 0>
constexpr bool valid(E const &e) {

  if (e.size() == typename E::size_type(1) && e[0] == typename E::size_type(1))
    return true;
...
auto to_string(E const &e) {
  ...
  std::string s = "{ ";
  auto one = typename E::size_type(1);
  for (auto i = typename E::size_type(0); i < e.size(); i++) {
    if (i != e.size() - one) {
      s += std::to_string( e.at(i) ) + ", ";
    }
  }
  s += std::to_string( e.at( e.size() - one ) ) + " }";
  return s;
}
...
auto squeeze(E const &e) {
  ...
  while (new_extent.size() < typename E::size_type(2))
    new_extent.push_back(typename E::value_type(1));
